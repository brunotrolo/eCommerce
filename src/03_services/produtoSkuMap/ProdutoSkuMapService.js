/**
 * ProdutoSkuMapService — pareamento entre anúncios da Shopee (ANUNCIOS_SHOPEE)
 * e produtos do estoque (ESTOQUE), usando o campo nativo item_sku da Shopee
 * como chave (sem tabela de mapeamento interna — a aba é reescrita pelo sync
 * a partir do item_sku real).
 *
 * Absorveu (12/08/2026) as ações de sustentação do antigo domínio
...
var ProdutoSkuMapService = (function () {
  ...
  function logSalesFailure_(itemId, error) {
    try {
      LoggingService.log({
        service: 'produtoSkuMap',
        action: 'fetchSales',
        status: 'ERROR',
        caller: 'ProdutoSkuMapService.fetchSalesBatch_',
        summary: 'shopee_sales_by_item falhou para o item ' + itemId,
        errorMessage: (error && error.message) ? error.message : String(error)
      });
    } catch (logErr) {
      console.error('[ProdutoSkuMap] Falha ao logar erro de vendas: ' + (logErr.message || logErr));
    }
  }
  ...
  function syncListings(params) {
    ...
    if (items.length > 0) {
      try {
        ...
        var result = ProdutoSkuMapRepository.syncMain(sheetId, items);
        ...
        ProdutoSkuMapRepository.writePerformance(sheetId, {
  ...
  function logDiffsFromSync_(sheetId, newRows) {
    var oldRows = ProdutoSkuMapRepository.getAll(sheetId);
    var oldMap = {};
    ...
    for (var j = 0; j < newRows.length; j++) {
      ...
      if (oldPrice !== newPrice && newPrice > 0) {
        ProdutoSkuMapRepository.logPrecoChange(sheetId, {
          ITEM_ID: row.ITEM_ID,
          NOME_ITEM: row.NOME,
      ...
      if (oldStock !== newStock) {
        ProdutoSkuMapRepository.logEstoqueChange(sheetId, {
          ITEM_ID: row.ITEM_ID,
          NOME_ITEM: row.NOME,
  ...
  function updateSku(params) {
    ...
    ProdutoSkuMapRepository.patchMain(sheetId, itemId, {
      SKU: confirmedSku,
  ...
  function getSugestoes() {
    ...
    var anuncios = ProdutoSkuMapRepository.getAll(sheetId) || [];
